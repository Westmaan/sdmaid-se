    ...
    override suspend fun initialize() {
        ...
        TARGET_APPS.forEach { (pkg, folderName) ->
            ...
            pathMappings.forEach { (area, basePath) ->
                ...
                // Add standard media types
                MEDIA_TYPES.forEach { type ->
                    criteria.add(SegmentCriterium("$mediaBase $type", SegmentCriterium.Mode.Ancestor()))
                }

                // Add specialized logic for Voice Notes
                criteria.add(
                    CriteriaOperator.And(
                        SegmentCriterium("$mediaBase Voice Notes", SegmentCriterium.Mode.Ancestor()),
                        SegmentCriterium(
