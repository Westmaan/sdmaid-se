...
class AdvertisementFilter @Inject constructor(
    ...
    override suspend fun match(
        ...
        if (size >= 2 && BLACKLIST_AREAS.contains(areaType)) {
            ...
            if (pkgId.name.equals(pfpSegs[0], ignoreCase = true) && 
                cacheFolderPrefixes.any { it.equals(segment1, ignoreCase = true) }) {
                return null
            }
        }

        if (size > 0) {
            val lastSegment = pfpSegs[size - 1]
            if (IGNORED_FILES.contains(lastSegment.lowercase())) {
                return null
            }
        }
        ...
        if (size >= 3) {
            val segment1 = pfpSegs[1]
            if (HIDDEN_CACHE_FOLDERS.contains(segment1.lowercase())) {
        ...
        if (size >= 4) {
            ...
            if (SEGMENT_FILES.equals(segment1, ignoreCase = true) && 
                HIDDEN_CACHE_FOLDERS.contains(segment2.lowercase())) {
                return target.toDeletionMatch()
            }
        }

        if (size >= 2) {
            val subDir = pfpSegs.subList(1, size)
    ...
    companion object {
        ...
        private val HIDDEN_CACHE_FOLDERS = setOf(
            "vast_rtb_cache",
            "goadsdk",
            "iflyadimgcache",
        )
