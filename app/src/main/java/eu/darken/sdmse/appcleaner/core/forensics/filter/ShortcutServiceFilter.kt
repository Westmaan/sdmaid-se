...
class ShortcutServiceFilter @Inject constructor(
    ...
    override suspend fun match(
        ...
        // 2. Fail Fast: Check Path Depth
        // pfpSegs: ["shortcut_service", "bitmaps", "{packageName}", "file.png", ...]
        if (pfpSegs.size < 4) return null

        // 3. Robust Checks: Verify path structure (Case Insensitive)
        if (!SEGMENT_SHORTCUT_SERVICE.equals(pfpSegs[0], ignoreCase = true)) return null
        if (!SEGMENT_BITMAPS.equals(pfpSegs[1], ignoreCase = true)) return null

        // 4. Verify Package Ownership
        if (!pkgId.name.equals(pfpSegs[2], ignoreCase = true)) return null
    ...
    companion object {
        ...
        private const val SEGMENT_SHORTCUT_SERVICE = "shortcut_service"
        private const val SEGMENT_BITMAPS = "bitmaps"
    }
}
